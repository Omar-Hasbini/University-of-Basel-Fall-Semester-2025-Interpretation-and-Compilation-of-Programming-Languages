...
\begin{array}{rcl}
\Reg &::=& \allregisters{} \\
\Dst &::=&  \key{\%}\Reg \MID \Int\key{(}\key{\%}\Reg\key{)}\\
\Src &::=&  \key{\$}\Int \MID \Dst \\
\Instr &::=& \key{addq} \; \Src\key{,} \Dst \MID
      \key{subq} \; \Src\key{,} \Dst \MID
      \key{negq} \; \Dst \MID \key{movq} \; \Src\key{,} \Dst \MID \\
  &&  \key{pushq}\;\Dst \MID \key{popq}\;\Dst \MID 
...
\begin{array}{lcl}
\Reg &::=& \allregisters{} \\
\Dst &::=&  \REG{\Reg} \MID \DEREF{\Reg}{\Int} \\
\Src &::=&  \IMM{\Int} \MID \Dst \\
\Instr &::=& \BININSTR{\code{addq}}{\Src}{\Dst} 
       \MID \BININSTR{\code{subq}}{\Src}{\Dst}\\
       &\MID& \UNIINSTR{\code{negq}}{\Dst}
       \MID \BININSTR{\code{movq}}{\Src}{\Dst}\\
       &\MID& \PUSHQ{\Dst}
       \MID \POPQ{\Dst} \\
...
\Dst &::=&  \REG{\Reg} \MID \DEREF{\Reg}{\Int} \\
\Src &::=&  \IMM{\Int} \MID \Dst \\
\Instr &::=& \BININSTR{\skey{addq}}{\Src}{\Dst} 
       \MID \BININSTR{\skey{subq}}{\Src}{\Dst}\\
       &\MID& \UNIINSTR{\skey{negq}}{\Dst}
       \MID \BININSTR{\skey{movq}}{\Src}{\Dst}\\
       &\MID& \PUSHQ{\Dst}
       \MID \POPQ{\Dst} \\
      &\MID& \CALLQ{\itm{label}}{\itm{int}}
       \MID \RETQ{}  \\
...
to the \Dst{} nonterminal of the \LangXInt{} abstract syntax
...
operation, on the left side.  (Let $\Src_1$ and $\Src_2$ be the
translations of $\Atm_1$ and $\Atm_2$, respectively.)  There is an
\key{addq} instruction in x86, but it performs an in-place update.
%
So, we could move $\Src_1$ into the \code{rax} register, then add
$\Src_2$ to \code{rax}, and then finally move \code{rax} into \itm{var}. 
\begin{transformation}
{\if\edition\racketEd    
\begin{lstlisting}
|$\itm{var}$| = (+ |$\Atm_1$| |$\Atm_2$|);
...
left-hand side, then we can move $\Src_1$ into the left-hand \itm{var}
and then add $\Src_2$ to \itm{var}.
%
\begin{transformation}
{\if\edition\racketEd    
\begin{lstlisting}
...
$\Src$ into \code{rdi} and then call \code{print\_int} using the
...
of locations that appear in a \Src{} and \Dst, (2) compute the locations read
...
\TRUE{} and \FALSE{} to the \Atm{} nonterminal.  Regarding control
...
the first argument, as follows, where $\Src$ is the translation of
$\Atm$ to x86:
\[
\CASSIGN{\Var}{\CUNIOP{\key{not}}{\Atm}}
\qquad\Rightarrow\qquad
\begin{array}{l}
\key{movq}~ \Src\key{,} \Var\\
\key{xorq}~ \key{\$1,} \Var
\end{array}
\]
...
\Instr &::=& \key{xorq}~\Src\key{,}~\Dst
   \MID \key{cmpq}~\Src\key{,}~\Src
    \MID  \key{set}cc~\Dst 
    \MID \key{movzbq}~\Src\key{,}~\Dst \\
...
\itm{bytereg} &::=& \key{ah} \MID \key{al} \MID \key{bh} \MID \key{bl}
    \MID \key{ch} \MID \key{cl} \MID \key{dh} \MID \key{dl} \\
\Dst &::=&  \BYTEREG{\itm{bytereg}} \\
\itm{cc} & ::= & \key{e} \MID \key{l} \MID \key{le} \MID \key{g} \MID \key{ge} \\
\Instr &::=& \BININSTR{\code{xorq}}{\Src}{\Dst}
       \MID \BININSTR{\code{cmpq}}{\Src}{\Src}\\
       &\MID& \BININSTR{\code{set}}{\itm{cc}}{\Dst} 
       \MID \BININSTR{\code{movzbq}}{\Src}{\Dst}\\
...
\Dst &::=&  \gray{\REG{\Reg} \MID \DEREF{\Reg}{\Int}} 
     \MID \BYTEREG{\itm{bytereg}} \\
\Src &::=&  \gray{\IMM{\Int} \MID \Dst} \\
\itm{cc} & ::= & \skey{e} \MID \skey{ne} \MID \skey{l} \MID \skey{le} \MID \skey{g} \MID \skey{ge} \\
\Instr &::=& \BININSTR{\scode{xorq}}{\Src}{\Dst}
       \MID \BININSTR{\scode{cmpq}}{\Src}{\Src}\\
       &\MID& \UNIINSTR{\scode{set}\code{+}\itm{cc}}{\Dst} 
       \MID \BININSTR{\scode{movzbq}}{\Src}{\Dst}\\
...
semantics of x86. In the following translation, let $\Src$ be the
...
right-hand side into a sequence of three instructions. Let $\Src_1$
be the translation of $\Atm_1$ to x86 and likewise for $\Src_2$.\\
\begin{tabular}{lll}
\begin{minipage}{0.4\textwidth}
...
a regular jump (for the \emph{else} branch). Again, $\Src_1$ and $\Src_2$
...
\begin{array}{lcl}
  \Instr &::=& \key{andq}~\Src\key{,}~\Dst
          \MID \key{sarq}~\Src\key{,}~\Dst \\
  \Src &::=& \itm{label} \key{(\%rip)} 
\end{array}
}

\newcommand{\ASTXGlobalRacket}{
\begin{array}{lcl}
  \Instr &::=& \BININSTR{\scode{andq}}{\Src}{\Dst} \MID
               \BININSTR{\scode{sarq}}{\Src}{\Dst} \\
  \Src &::=&  \GLOBAL{\itm{label}} 
\end{array}
}
...
\begin{array}{l}
  ...
  \LfunMonadASTPython \\
  \begin{array}{rcl}
     \LangFunANFM{} &::=& \PROGRAM{}{\LS \Def \ldots \mainDef \RS}
  \end{array}
\end{array}
\]
\fi}
...
\begin{array}{lcl}
\Instr &::=&  \key{callq}\;\key{*}\Src \MID \key{tailjmp}\;\Src 
     \MID \key{leaq}\;\Src\key{,}\;\key{\%}\Reg \\
\Block &::= & \Instr^{+} \\
...
\begin{array}{lcl}
  \Instr &::=& \INDCALLQ{\Src}{\itm{int}}
    \MID \TAILJMP{\Src}{\itm{int}}\\
    &\MID& \BININSTR{\code{'leaq}}{\Src}{\REG{\Reg}}\\
  \Block &::= & \BLOCK{\itm{info}}{\LP\Instr\ldots\RP}\\
...
\begin{array}{lcl}
  \Dst &::=&  \gray{  \REG{\Reg} \MID \DEREF{\Reg}{\Int}
     \MID \BYTEREG{\Reg} } \\
  \Src &::=&  \gray{  \INT{\Int} \MID \Dst } \\
     &\MID& \gray{ \GLOBAL{\itm{label}} } \MID \FUNREF{\itm{label}}{\Int} \\
  \Instr &::=& \ldots \MID \INDCALLQ{\Src}{\itm{int}}
    \MID \TAILJMP{\Src}{\itm{int}}\\
    &\MID& \BININSTR{\scode{leaq}}{\Src}{\REG{\Reg}}\\
...
the translation of $\itm{lhs}$ from \Atm{} in \LangCFun{} to \Src{} in
...
$\RETURN{\Atm}$ can be compiled to an assignment to \code{rax} followed
